...
fn parseLine(line: []u8) !ScriptEntry {
    ...
    assert(rangeStartFound == true);
    assert(rangeEndIndex == 0);
    assert(scriptFound == true);
    assert(generalCategoryFound == true);
...
fn matchScript(scripts: std.ArrayList(ScriptEntry), codepoint: u21) ScriptEntry {
    ...
    std.debug.panic("failed to matche codepoint {}", .{codepoint});
}
