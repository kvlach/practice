...
fn matchScript(scripts: std.ArrayList(ScriptEntry), codepoint: u21) ScriptEntry {
	...
	std.debug.panic("unknown codepoint '{}'", .{codepoint});
}
...
pub fn main() !void {
	...
	codepointIter = (try std.unicode.Utf8View.init(s)).iterator();
	var cluster = false;
	...
	while (codepointIter.nextCodepoint()) |codepoint| {
		...
		const spacingMark = cat == GeneralCategory.Spacing_Mark or codepoint == 0x0E33 or codepoint == 0x0EB3; // + Grapheme_Cluster_Break ≠ Extend + specific character exceptions

		if (extend or zwj or spacingMark) {
			cluster = true;
		} else if (cluster) {
			std.debug.print(" {x}", .{codepoint});
			cluster = false;
