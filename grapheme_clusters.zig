...
fn parseLine(line: []u8) !ScriptEntry {
    ...
    for (0.., line) |i, c| {
        if (!rangeStartFound and c == ' ') {
            ...
            script = try parseScript(line[i + 2 ..]);
            scriptFound = true;
    ...
    std.debug.assert(rangeStartFound == true);
    std.debug.assert(scriptFound == true);
    std.debug.assert(generalCategoryFound == true);
...
pub fn main() !void {
    ...
    var codepoint_iter = (try std.unicode.Utf8View.init("aα👩‍🎨")).iterator();
