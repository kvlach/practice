...
fn matchScript(scripts: std.ArrayList(ScriptEntry), codepoint: u21) ScriptEntry {
	for (scripts.items) |script| {
		if (script.rangeStart == codepoint) {
			return script;
		} else if (script.rangeEnd) |rangeEnd| {
			if (script.rangeStart < codepoint and codepoint <= rangeEnd) {
				return script;
			}
		}
	}
	std.debug.panic("failed to match codepoint '{}'", .{codepoint});
}
...
pub fn main() !void {
	...
	while (codepointIter.nextCodepoint()) |codepoint| {
		...
		// const graphemeClusterBreak =
		// const spacingMark = cat == GeneralCategory.Spacing_Mark; // + Grapheme_Cluster_Break ≠ Extend, + other characters specified explicitly
