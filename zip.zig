...
const Bits = struct {
	...
	fn next(self: *Bits) BlockPart {
		if (self.expect_distance) {
			...
			return BlockPart.init(BlockParts.BackwardDistance, self.read(u5));
		}
		...
		return switch (n) {
			256 => BlockPart.init(BlockParts.EndOfBlock, 0),
			257...285 => {
				self.expect_distance = true;
				return BlockPart.init(BlockParts.Length, n);
			},
			else => BlockPart.init(BlockParts.LiteralByte, n),
		};
	}
};
...
const BlockPart = struct {
	...
	fn init(kind: BlockParts, value: i10) BlockPart {
		return BlockPart{
			.kind = kind,
			.value = value,
		};
