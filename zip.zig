...
const DeflateHuffmanHeaderBlockType = enum {
	block_is_stored,
	fixed_huffman_codes_for_literal_and_distance_codes,
	dynamic_huffman_codes,
	reserved,
};

const deflate_huffman_header_block_type_arr = [_]DeflateHuffmanHeaderBlockType{
	DeflateHuffmanHeaderBlockType.block_is_stored,
	DeflateHuffmanHeaderBlockType.fixed_huffman_codes_for_literal_and_distance_codes,
	DeflateHuffmanHeaderBlockType.dynamic_huffman_codes,
	DeflateHuffmanHeaderBlockType.reserved,
};

const DeflateHuffmanHeader = struct {
	last_block_bit: bool,
	block_type: DeflateHuffmanHeaderBlockType,

	fn init(bs: *Bytes) DeflateHuffmanHeader {
		const b = bs.u8le();

		const last_block_bit = b >> 7 == 0b1;
		const block_type = deflate_huffman_header_block_type_arr[(b>>5) & 0b11];

		bs.set(b & 0b11111);

		return DeflateHuffmanHeader {
			.last_block_bit = last_block_bit,
...
pub fn main() !void {
	...
	const deflate = DeflateHuffmanHeader.init(@constCast(&bs));
	std.debug.print("{}\n", .{deflate});
}
