...
const BlockPart = struct {
	...
	fn init(kind: BlockPartTypes, val: i10) BlockPart {
		return BlockPart{
			.kind = kind,
			.val = val,
		};
...
const Bits = struct {
	...
	fn next(self: *Bits) BlockPart {
		if (self.expect_distance) {
			...
			return BlockPart.init(BlockPartTypes.Distance, @as(i10,self.read(u5)));
		}
		const num_of_bits = self.number_of_bits_to_read();
		const to_add = self.number_to_add();
		const n = switch (num_of_bits) {
			7 => @as(i10, self.read(u7)),
			8 => @as(i10, self.read(u8)),
			9 => @as(i10, self.read(u9)),
			else => std.debug.panic("unreachable", .{}),
		} + to_add;

		return switch (n) {
			256 => BlockPart.init(BlockPartTypes.EndOfBlock, n),
			257...285 => {
				self.expect_distance = true;
				return switch (n) {
					257...264 => BlockPart.init(BlockPartTypes.Length, n),
					265...268 => BlockPart.init(BlockPartTypes.Length, n-254+self.read(u1)),
					269...272 => BlockPart.init(BlockPartTypes.Length, n-254+self.read(u2)),
					273...276 => BlockPart.init(BlockPartTypes.Length, n-254+self.read(u3)),
					277...280 => BlockPart.init(BlockPartTypes.Length, n-254+self.read(u4)),
					281...284 => BlockPart.init(BlockPartTypes.Length, n-254+self.read(u5)),
					285 => BlockPart.init(BlockPartTypes.Length, 258),
					else => std.debug.panic("unreachable", .{}),
				};
			},
			else => BlockPart.init(BlockPartTypes.Literal, n),
		};
	}
};
