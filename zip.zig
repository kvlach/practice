...
const DeflateHeaderBlockType = enum {
	block_is_stored,
...
const DeflateHeader = struct {
	...
	fn init(bs: *Bytes) DeflateHeader {
		const n = bs.u8le();
		const deflate = DeflateHeader {
			.last_block_bit = n >> 7 == 1,
...
const HuffmanNode = struct {
	...
	fn init(symbol: u16, priority: f64) HuffmanNode {
		return HuffmanNode {
			.symbol = symbol,
			.priority = priority,
		};
...
pub fn main() !void {
	...
	const deflate = DeflateHeader.init(@constCast(&bs));
	std.debug.print("{}\n", .{deflate});
}
