...
const Bits = struct {
	...
	fn byte_to_bits(byte: u8) [8]u1 {
		...
		const g: u1 = @intCast((byte >> 5) & 0b1);
		const c: u1 = @intCast((byte >> 4) & 0b1);
		const d: u1 = @intCast((byte >> 3) & 0b1);
		const e: u1 = @intCast((byte >> 2) & 0b1);
		const f: u1 = @intCast((byte >> 1) & 0b1);
	...
	fn next(self: *Bits) BlockPart {
		if (self.expect_distance) {
			...
			return BlockPart.init(BlockParts.Distance, self.read(u5));
		}
		...
		return switch (n) {
			...
			257...285 => {
				...
				return BlockPart.init(BlockParts.Length, n);
				// return switch (n) {
				// 	257...264 => BlockPart.init(BlockParts.Length, n-254),
				// 	265...268 => BlockPart.init(BlockParts.Length, n-254+self.read(u1)),
				// 	269...272 => BlockPart.init(BlockParts.Length, n-254+self.read(u2)),
				// 	273...276 => BlockPart.init(BlockParts.Length, n-254+self.read(u3)),
				// 	277...280 => BlockPart.init(BlockParts.Length, n-254+self.read(u4)),
				// 	281...284 => BlockPart.init(BlockParts.Length, n-254+self.read(u5)),
