...
pub fn main() !void {
	...
	while (codepointIter.nextCodepoint()) |codepoint| {
		...
		const graphemeExtend = cat == GeneralCategory.Nonspacing_Mark || GeneralCategory.Enclosing_Mark; // TODO: + Other_Grapheme_Extend
		const emojiModifier = false; // TODO
		const extend = graphemeExtend || emojiModifier;
		const zwj = codepoint == 0x200D;
		const spacingMark = cat == GeneralCategory.Spacing_Mark;
	}
}
